# Claude Code Project Instructions — reqwest-kotlin

## Project Overview

This is **reqwest-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`reqwest`](https://crates.io/crates/reqwest).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/reqwest/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/reqwest>
